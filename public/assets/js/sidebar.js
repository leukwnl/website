(() => {
    ...
    // Inject sidebar partial if a placeholder is present
    const placeholder = document.querySelector('[data-sidebar-partial]');
    if (placeholder) {
        fetch('/public/assets/partials/sidebar-content.html', { cache: 'no-cache' })
            .then(r => r.ok ? r.text() : '')
            .then(html => { if (html) { placeholder.innerHTML = html; } })
            .catch(() => { });
    }
})();
